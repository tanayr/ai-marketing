...
export function CreateInspirationForm({ onSuccess, onCancel }: CreateInspirationFormProps) {
  ...
  const onSubmit = async (values: InspirationFormValues) => {
    try {
      ...
      const response = await fetch("/api/super-admin/creative-inspirations", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to create inspiration");
      }
